...
def tf_predict(classifier, image):
	...
	pred_input_fn = tf.estimator.inputs.numpy_input_fn(x={"p":processed_array}, shuffle=False)
	...
	while True:
		...
		if len(contours) > 0:
			...
			if cv2.contourArea(contour) > 10000:
				p1,q1,r1,s1 = cv2.boundingRect(contour)
				save_img = thresh[y1:y1+h1, x1:x1+w1]
				
				if r1 > s1:
					save_img = cv2.copyMakeBorder(save_img, int((r1-s1)/2) , int((r1-s1)/2) , 0, 0, cv2.BORDER_CONSTANT, (0, 0, 0))
				elif s1 > r1:
					save_img = cv2.copyMakeBorder(save_img, 0, 0, int((s1-r1)/2) , int((s1-r1)/2) , cv2.BORDER_CONSTANT, (0, 0, 0))
